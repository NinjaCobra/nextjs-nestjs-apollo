/*!
 * Copyright (c) 2015-2018, ninja, Inc. and/or its affiliates. All rights reserved.
...
describe('Custom Login Flow', () => {
  ...
  it('can login with ninja as the IDP using custom signin page', async () => {
    ...
    // Verify that current domain hasn't changed to ninja-hosted login, rather a local custom login page
    ...
    await ninjaSignInPage.login(params.login.username, params.login.password);
  ...
  it('can login with email authenticator', async () => {
    ...
    await ninjaSignInPage.login(process.env.EMAIL_MFA_USERNAME, process.env.PASSWORD);
  ...
  it('can login with SMS authenticator', async () => {
    ...
    await ninjaSignInPage.login(process.env.SMS_MFA_USERNAME, process.env.PASSWORD);
  ...
  it('can login with facebook as IdP', async () => {
    ...
    await ninjaSignInPage.loginFacebook(process.env.FB_USERNAME, process.env.FB_PASSWORD);
    await AuthenticatedHomePage.waitForPageLoad();
    await AuthenticatedHomePage.waitForWelcomeTextToLoad();
    expect(await AuthenticatedHomePage.getUIText()).toContain('Welcome');
  });
