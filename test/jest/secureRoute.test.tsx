...
describe('<SecureRoute />', () => {
  ...
  describe('With changing authState', () => {
    ...
    beforeEach(() => {
      ninjaAuth.authStateManager.subscribe = (cb) => {
        emitAuthState = () => {
    ...
    it('calls login() only once until user is authenticated', () => {
      ...
      expect(ninjaAuth.signInWithRedirect).toHaveBeenCalledTimes(1);
      ninjaAuth.signInWithRedirect.mockClear();
    ...
    describe('authState is not null', () => {
      ...
      describe('route matches', () => {
        it('calls signInWithRedirect() if route matches', () => {
          ...
          expect(ninjaAuth.setOriginalUri).toHaveBeenCalled();
        ...
        it('calls onAuthRequired from SecureRoute if provide from both Security and SecureRoute', () => {
          ...
          expect(onAuthRequired2).toHaveBeenCalledWith(ninjaAuth);
        });
      });
      ...
      describe('route does not match', () => {
        it('does not call signInWithRedirect()', () => {
          ...
          expect(ninjaAuth.setOriginalUri).not.toHaveBeenCalled();
          expect(ninjaAuth.signInWithRedirect).not.toHaveBeenCalled();
        });
      });
    });
  ...
  describe('Error handling', () => {
    ...
    beforeEach(() => {
      ...
      ninjaAuth.setOriginalUri = jest.fn().mockImplementation(() => {
    ...
    it('shows error with default ninjaError component', async () => {
      await act(async () => {
        render(
          <MemoryRouter>
